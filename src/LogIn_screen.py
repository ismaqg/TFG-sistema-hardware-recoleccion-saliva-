...
class LogIn_screen():  # singleton
    ...
    def __try_to_logIn(self):
        ...
        if (LogIn_screen.__is_a_valid_TSI(self.__input_variable.get())):
            ...
            if person.get_status() == "ADMIN" or person.get_status() == "OPERATOR":
                    ...
                    Screen_manager.get_root().after(500, lambda:self.__check_2nd_authentication_response_and_logIn_if_correct(additional_security_check, person))  # here the login will succeed if the password is correct
            else: #USER
                if Checker.check_available_resources() == False:  # cannot login because there are not available resources
                    ...
                    ActivePerson.destroyCurrent()  # we instantiated an activePerson some lines above
    ...
    def __check_2nd_authentication_response_and_logIn_if_correct(self, additional_security_check, person):
        if (not additional_security_check.key_introduced()):   # NOTE: This will be false if key was NOT introduced of if the keyboard was closed (in these cases, the timer wont be reprogrammed)
